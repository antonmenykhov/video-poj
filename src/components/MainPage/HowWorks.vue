...
<section class="how-works flex">
    <div class="how-works-left">
        <div class="up-header">О компании</div>
        <h3 class="title white">Как мы работаем</h3>
        <div class="description white">За время работы, мы выработали четкий алгоритм, по которому мы взаимодействуем с заказчиками</div>
        <div class="time-line">
            <div class="time-line-item" v-for="item,i in timeline" :key="i">
                <div class="time-line-num">0{{i+1}}</div>
                <div class="time-line-item-text">{{item.name}}</div>
            </div>
        </div>
    </div>
    <div class="how-works-right">
        <div class="up-header brown">Получить услугу</div>
        <h3 class="title">Отправить запрос</h3>
        <div class="description">Нужна консультация по услугам? <br> Оставьте заявку и мы перезвоним вам в ближайшее время</div>
        <form>
            <div class="line flex">
                <div class="input">
                ...
                <div class="input">
                    <div class="label-input">Ваш номер телефона</div>
            ...
            <div class="line">
                <div class="input input-service">
                    ...
                    <select  v-model="form.service" type="text" name="service">
                        <option v-for="item,i in services" :key="i" :value="item">{{item}}</option>
                    </select>
                </div>
            </div>
            <button @click="send" class="orange-button brown-button">Отправить</button>
        </form>
    </div>
</section>
</template>
